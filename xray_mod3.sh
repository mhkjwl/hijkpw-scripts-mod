...
validateConfig() {
    if [[ ! -x /usr/local/bin/xray ]]; then
        colorEcho $RED " Xray程序不存在，无法验证配置"
        return 1
    fi

    local test_log="/tmp/xray-config-test.log"
    if ! /usr/local/bin/xray run -test -config "$CONFIG_FILE" >"$test_log" 2>&1; then
        colorEcho $RED " Xray配置验证失败，未启动服务"
        cat "$test_log"
        rm -f "$test_log"
        return 1
    fi
...
getConfigFileInfo() {
    ...
    if [[ "$vmess" = "" ]]; then
        ...
        if [[ "$flow" = "xtls-rprx-vision" ]]; then
            vision="true"
            xtls="true"
        elif grep -q xtlsSettings $CONFIG_FILE; then
            xtls="true"
            [[ -z "$flow" ]] && flow="旧XTLS流控"
        elif [[ -z "$flow" ]]; then
            flow="无"
...
showInfo() {
    ...
    if [[ "$vless" = "false" ]]; then
        ...
        if [[ "$xtls" = "true" ]]; then
            ...
            echo -e " ${BLUE}伪装域名/主机名(host)/SNI/peer名称：${PLAIN}${RED}${domain}${PLAIN}"
            if [[ "$vision" = "true" ]]; then
                echo -e " ${BLUE}底层安全传输(tls)：${PLAIN}${RED}TLS + Vision${PLAIN}"
            else
                echo -e " ${BLUE}底层安全传输(tls)：${PLAIN}${RED}旧XTLS${PLAIN}"
            fi
        elif [[ "$ws" = "false" ]]; then
            echo -e " ${BLUE}IP(address):  ${PLAIN}${RED}${IP}${PLAIN}"
            echo -e " ${BLUE}端口(port)：${PLAIN}${RED}${port}${PLAIN}"
...
menu() {
    ...
    echo -e "  ${GREEN}8.${PLAIN}   安装Xray-${BLUE}VLESS+TCP+TLS+Vision${PLAIN}${RED}(推荐)${PLAIN}"
